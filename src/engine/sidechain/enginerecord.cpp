...
#include "util/sample.h" //Using MixMultiChanneltomono fucntion to convert stereo to mono 
...
EngineRecord::EngineRecord(UserSettingsPointer pConfig)
        : m_pConfig(pConfig),
          ...
          m_iMetaDataLife(0),
          m_cueTrack(0),
          m_recordChannels(2), //defaulting to stereo but not picking from config in case config is not yet loaded
...
int EngineRecord::updateFromPreferences() {
    ...
    int channelMode = m_pConfig->getValue<int>(ConfigKey(RECORDING_PREF_KEY, QStringLiteral("channel_mode")), 0);
    if (channelMode == 1) {
